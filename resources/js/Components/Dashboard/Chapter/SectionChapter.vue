<script setup>
    ...
    const idImage = 'idImage_' + props.chapter.id
    const idIcon = 'idIcon_' + props.chapter.id

    const form = useForm({
        id: props.chapter.id,
        catalog_id: props.chapter.catalog_id,
        title: props.chapter.title,
        slug: props.chapter.slug,
        sort: props.chapter.sort,
        image: null,
        icon: null,
        _method: 'POST',
    })
    ...
    const onReset = () => {
        form.id = props.chapter.id
        form.title = props.chapter.title
        form.slug = props.chapter.slug
    ...
    const urlImage = () =>  form.image != undefined ? URL.createObjectURL(form.image) : props.chapter.image
    const urlIcon = () =>  form.icon != undefined ? URL.createObjectURL(form.icon) : props.chapter.icon
                ...
                <q-list class="img-width">
                    <q-item class="no-padding">
                        ...
                        <q-item-section class="items-between col-2">
                            ...
                            <q-btn label="Очистить" type="reset" color="gries" flat no-caps class="q-ml-sm" />
                            <q-btn v-if="chapter.id" label="Удалить" color="dbrem" class="q-ml-sm" @click.stop.prevent="onDelete" flat no-caps />
                        </q-item-section>
                    </q-item>
                </q-list>
            </q-card-section>
